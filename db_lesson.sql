...
Q4
UPDATE people SET department_id = 3 WHERE department_id IS NULL AND age < 30;
UPDATE people SET department_id = 4 WHERE department_id IS NULL AND age > 30;
UPDATE people SET department_id = 5 WHERE gender IS NULL;
...
SELECT name
FROM people
WHERE 20 <= age AND age < 30 AND gender = 2
OR 40 <= age AND age < 50 AND gender = 1;
...
Q10
SELECT d.name, p.name, r.content
FROM departments d
INNER JOIN people p ON d.department_id = p.department_id
